...
def  check_income_range(df:pd.DataFrame) -> bool:
    invalid= df[(df['Income'] < 0) | (df['Income'] > 600000)]
    if len(invalid) > 0:
        print(f"FAIL: {len(invalid)} rows with invalid income range found")
        return False
